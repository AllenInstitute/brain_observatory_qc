...
def noise_std(x: np.ndarray, filter_length: int = 31) -> float:
    ...
    if any(np.isnan(x)):
        return np.NaN
    noise = x - percentile_filter(x, 50, filter_length) # same as median filter,
    # with mode='reflect'
...
def calculate_dff(corrected_trace, baseline, noise_sd):
    ...
    1d array
        dff
    """
    dff = ((corrected_trace - baseline)
        / np.maximum(baseline, noise_sd))
    return dff
...
def get_new_dff_df(ophys_experiment_id, inactive_kernel_size = 30, inactive_percentile = 10):
    ...
    new_dff_all = []
    old_dff_all = []
    dff_h = h5py.File(from_lims.get_dff_traces_filepath(ophys_experiment_id), 'r')
    for _, row in np_corrected_df.iterrows():
        corrected_trace = row.np_corrected
        crid = row.cell_roi_id
        # Calculate noise_sd
        noise_sd = noise_std(corrected_trace, filter_length=int(round(frame_rate * 3.33))) # 3.33 s is fixed
        ...
        active_frame = np.where(corrected_trace > (low_baseline + 3 * noise_sd))[0]
        ...
        # Load old dff
        roi_ind = np.where([int(rn) == crid for rn in dff_h['roi_names']])[0][0]
...
def get_fixed_r_values(ophys_experiment_id, crid_values, num_normal_r_thresh, r_replace):
    ...
    r_out_of_range = []
    num_normal_r = 0
    np_correction_h = h5py.File(from_lims.get_neuropil_correction_filepath(ophys_experiment_id), 'r')
    for crid in crid_values:
        roi_ind = np.where([int(rn) == crid for rn in np_correction_h['roi_names']])[0][0]
...
def get_np_corrected_df(ophys_experiment_id, num_normal_r_thresh = 5, r_replace=0.7):
    ...
    r_list, r_out_of_range = get_fixed_r_values(ophys_experiment_id, crid_values, num_normal_r_thresh, r_replace)
    
    np_corrected_all = []
    demixed_h = h5py.File(from_lims.get_demixed_traces_filepath(ophys_experiment_id), 'r')
    neuropil_h = h5py.File(from_lims.get_neuropil_traces_filepath(ophys_experiment_id), 'r')
    for crid, r in zip(crid_values, r_list):
        # Get the demixed trace, neuropil trace, neuropil correction coefficient and error
        roi_ind = np.where([int(rn) == crid for rn in demixed_h['roi_names']])[0][0]
        demixed = demixed_h['data'][roi_ind]
        
        roi_ind = np.where([int(rn) == crid for rn in neuropil_h['roi_names']])[0][0]
...
def draw_fig_new_dff(save_dir, new_dff_df, timestamps, oeid):
    for _, row in new_dff_df.iterrows():
        crid = row.cell_roi_id
        if os.path.isdir(save_dir / f'fig_new_dff_{oeid}') != True:
            os.mkdir(save_dir / f'fig_new_dff_{oeid}')
        fig, ax = plt.subplots(3,1,figsize=(12,9))
        ax[0].plot(timestamps, row.np_corrected, 'g', label='corrected', linewidth=0.5)
        ax[0].legend()
        ax[1].plot(timestamps, row.old_dff, 'k', label='old dff', linewidth=0.5)
        ax[1].legend()
        ax[2].plot(timestamps, row.new_dff, 'b', label='new dff', linewidth=0.5)
        ax[2].legend()
        ax[2].set_xlabel('Time (s)')
        fig.suptitle(f'Ophys experiment ID: {oeid}  Cell ROI ID: {crid}')
        fig.tight_layout()
        fig.savefig(save_dir / f'fig_new_dff_{oeid}' / f'{crid}.png', dpi = 300, bbox_inches='tight')
        plt.close(fig)